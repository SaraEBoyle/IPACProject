function [] = graph_behavior_Ale_V4()
...
%lick times are built off alt start times. cant use trial timestamp
    ...
    combine_spouts = 0;
    do_in_order = 0;
    left_spout_licks = 'Port3In';
    right_spout_licks = 'Port4In';
    left_fluid = 'tastant';
    right_fluid = 'water';
    left_window = 0;
    right_window = 3;
    ...
    %off_trial_number = 0;
    go_to = selpath{1,1};
    file = go_to(length(mouse_folder) + 1: end);
    path = mouse_folder;
    bpod_date = file(end-18:end - 4);
    ...
    if ~combine_spouts
        deleted_trials_inds = [];
        left_data = horzcat(vol_name, 'left alignment data.mat');
        ...
        right_data = horzcat(vol_name, 'right alignment data.mat');
    ...
    for u = sides_to_do
        ...
        if ~exist('Behavior Figures/', 'dir')
            ...
            if ~exist(horzcat('Behavior Figures/', horzcat(vol_name, result_name)), 'dir')
                mkdir(horzcat('Behavior Figures/', horzcat(vol_name, result_name)));
        ...
        elseif exist('Behavior Figures', 'dir')
            ...
            folderInfo = dir('Behavior Figures');
        end
        %Plot lick data
        if 1
            licks_per_session = {};
            ...
            if and(or(~isempty(both_event_trial_ind), ~isempty(left_event_trial_ind)), u == 1)
                ...
                save(horzcat(vol_tit, ' left lick data.mat'), 'left_lps_pre_5', 'left_lps_first_5', 'left_lps_second_5', 'left_lps_third_5', 'mean_left_lps_pre_5', 'mean_left_lps_first_5', 'mean_left_lps_second_5', 'mean_left_lps_third_5');
            ...
            if and(or(~isempty(right_event_trial_ind), ~isempty(both_event_trial_ind)), u == 2)
                ...
                if plot_non_dispensing_spout
                    ...
                    save(horzcat(vol_name, ' right lick data.mat'), 'right_lps_pre_5', 'right_lps_first_5', 'right_lps_second_5', 'right_lps_third_5', 'mean_right_lps_pre_5', 'mean_right_lps_first_5', 'mean_right_lps_second_5', 'mean_right_lps_third_5');
            ...
            if 1
                savefig(h, horzcat('Behavior Figures/',horzcat(vol_name, result_name), '/', num2str(volume_to_check), ' uL ', fluid, '_Lick_Data_Plots.fig'));
                ...
                if ~combine_spouts
                    ...
                    save(horzcat('Behavior Figures/',horzcat(vol_name, result_name), '/', num2str(volume_to_check), ' uL ', fluid, ' ', names{u}, ' lick rate data.mat'), 'avg_left_lps_within_window', 'avg_right_lps_within_window', 'left_licks_within_window_per_trial', 'right_licks_within_window_per_trial', 'left_lps_within_window_per_trial', 'right_lps_within_window_per_trial', 'total_licks_within_window_per_trial', 'total_lps_within_window_per_trial', 'total_lps_within_window', 'total_licks_within_window', 'window');
                end
