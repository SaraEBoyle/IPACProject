function [] = find_lick_z_score_correlationV2(do_AUC)
%% Finds the lick rates per trial
    ...
    for r = 1:size(signal_folder, 2)
        ...
        last = parens(end);
        temp_name = signal_name(last:end);
        B = regexp(temp_name,'\d*','Match');
        conv = str2double(B);
        
        signal_vol = num2str(conv);
        %if ~strcmp(signal_name(parens(end) + 2), ' ')
        %    signal_vol = horzcat(signal_vol, signal_name(parens(end) + 2));
        %end
        ...
        for k = 3 : length(signal_subFolders)
            ...
            signal_z_scores{z_ind} = thing_to_do; 
