function remove_high_artifact_trials_V4(left_window, right_window, fps)
close all
%% You can set these
...
for o = 1:size(all_signal_paths, 2)
    ...
    high_threshold_left = 2.7*std_left;
    low_threshold_left = -2.7*std_left;
    ...
    if ~strcmp(is_good, 'n')
        ...
        if ~strcmp(vol_name(2), ' ')
            volume = vol_name(1:2);
        end
        if or(strcmp(volume, 's'), strcmp(volume, 'c'))
            volume = '';
        else
            volume = horzcat(volume, ' ');
        end
        ...
        volume = strrep(vol_name, ' uL signal', '');
        volume = horzcat(volume, ' ');
        save(horzcat(volume, side_name, ' alignment data.mat'),'deleted_trials_inds','-append')
    ...
    close all
    baseline_correct = mean(mean(dFF_trials(:,1:60)));
    ...
    save(horzcat(signal_path, '/artifacts_removed/', 'z_scores.mat'), 'corrected_z_scores', 'control_corrected_z_scores', 'avg_z_scores', 'z_peaks','z_AUC', 'mean_z_AUC', 'std_AUC', 'z_mean', 'avg_control_z_scores', 'US_target_frame', 'x_axis')
    close all;
