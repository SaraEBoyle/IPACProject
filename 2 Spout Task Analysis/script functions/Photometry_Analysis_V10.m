...
function  Photometry_Analysis_V10(to_analyze, right_spout, left_spout, left_lick_port, right_lick_port, pre_time, post_time, seconds_per_trial, seconds_to_extend, trials_to_check, fiber, volume_to_check, pre_ITI_length, trigger1, frame_rate, leave_out_no_licks, lick_window, signal_key_word, analog_key_word, bpod_key_word, moving_avg, correct_for_bleaching)
    %% Clear Workspace
    %clear, clc
    for signal = 1:2
    close all
    ...
    [gcamp, iso] = correct_bleaching(horzcat(file, fiber_name, '/'), correct_for_bleaching, gcamp, iso, moving_avg, frame_rate, trigger1, 0);
    ...
    for iteration = 1:size(to_analyze, 2)
        ...
        [event_times, event_end_times, event_trial_ind, no_event_trial_ind, event_labels] = find_event_times(SessionData, alt_start_times, alignment, trials_to_check, volume_to_check, volume_to_check, leave_out_no_licks, stim_time, sound_on, outcomes, left_lick_port, right_lick_port, lick_window);
