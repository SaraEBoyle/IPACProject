...
function  Photometry_Analysis_V11(to_analyze, right_spout, left_spout, left_lick_port, right_lick_port, pre_time, post_time, seconds_per_trial, seconds_to_extend, trials_to_check, fibers, fiber_names, volume_to_check, pre_ITI_length, trigger1, frame_rate, leave_out_no_licks, lick_window, signal_key_word, analog_key_word, bpod_key_word, moving_avg, correct_for_bleaching)
    ...
    for fiber = fibers
            ...
            if plot_heatmap_and_average
                ...
                if single_volume == 1000
                   %legacy code
                    ...
                    x_axis = x_axis - laser_time;
                    if strcmp(alignment, 'right')
                        save('dFF_data.mat', 'right_trials', 'US_target_frame', 'CS_target_frame', 'x_axis', 'baseline_gcamp', 'baseline_iso');
                    elseif strcmp(alignment, 'left')
                        save('dFF_data.mat', 'left_trials', 'US_target_frame', 'CS_target_frame', 'x_axis', 'baseline_gcamp', 'baseline_iso');
                    else
                        save('dFF_data.mat', 'US_trials', 'no_US_trials', 'US_target_frame', 'CS_target_frame', 'x_axis', 'baseline_gcamp', 'baseline_iso');
                    end

                    %Use the same baselines used for bleaching correction
                    if signal == 1
                        baseline_gcamp_combo = cat(1,baseline_gcamp{:});
                        baseline_trials = baseline_gcamp_combo*100;
                    else
                        baseline_iso_combo = cat(1,baseline_iso{:});
                        baseline_trials = baseline_iso_combo*100;
                    end
                    std_avg = std(baseline_trials);
                    avg = mean(baseline_trials);
                    diffs = (US_trials - avg);
        ...
        %random_message(2); %this was an easter egg that I included to
        %insert a random inspirational quote to confuse the postdocs I collaborate 
        %with. Only activated every 20 times the code was run.
        end
        end
    end
end
